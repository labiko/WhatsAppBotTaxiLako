...
CREATE TABLE public.mouvements_balance (
  id uuid NOT NULL DEFAULT uuid_generate_v4(),
  entreprise_id uuid,
  paiement_id uuid,
  type_mouvement text NOT NULL CHECK (type_mouvement = ANY (ARRAY['paiement'::text, 'ajustement'::text, 'annulation'::text, 'correction'::text])),
  montant numeric NOT NULL,
  balance_avant numeric,
  balance_apres numeric,
  description text,
  created_at timestamp without time zone DEFAULT now(),
  CONSTRAINT mouvements_balance_pkey PRIMARY KEY (id),
  CONSTRAINT mouvements_balance_entreprise_id_fkey FOREIGN KEY (entreprise_id) REFERENCES public.entreprises(id),
  CONSTRAINT mouvements_balance_paiement_id_fkey FOREIGN KEY (paiement_id) REFERENCES public.paiements_entreprises(id)
);
...
CREATE TABLE public.paiements_entreprises (
  id uuid NOT NULL DEFAULT uuid_generate_v4(),
  entreprise_id uuid,
  periode_id uuid,
  montant_paye numeric NOT NULL CHECK (montant_paye > 0::numeric),
  methode_paiement text NOT NULL CHECK (methode_paiement = ANY (ARRAY['mobile_money'::text, 'virement'::text, 'especes'::text])),
  reference_paiement text,
  numero_beneficiaire text,
  montant_reversement numeric DEFAULT 0,
  montant_compensation numeric DEFAULT 0,
  statut text DEFAULT 'en_attente'::text CHECK (statut = ANY (ARRAY['en_attente'::text, 'confirme'::text, 'echec'::text, 'annule'::text])),
  ...
  created_at timestamp without time zone DEFAULT now(),
  updated_at timestamp without time zone DEFAULT now(),
  CONSTRAINT paiements_entreprises_pkey PRIMARY KEY (id),
  CONSTRAINT paiements_entreprises_periode_id_fkey FOREIGN KEY (periode_id) REFERENCES public.facturation_periodes(id),
  CONSTRAINT paiements_entreprises_entreprise_id_fkey FOREIGN KEY (entreprise_id) REFERENCES public.entreprises(id)
);
