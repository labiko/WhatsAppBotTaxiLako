-- Script de nettoyage et mise à jour pour conducteurs existants (version corrigée)
...
-- 2. Insérer les nouveaux conducteurs avec positions Paris et numéros français
...
-- 3. S'assurer que tous les conducteurs sont disponibles
UPDATE conducteurs SET 
  statut = 'disponible',
  actif = TRUE,
  derniere_activite = NOW();

-- 4. Vérification : compter les conducteurs par type
SELECT 
  vehicle_type,
...
-- 5. Afficher la liste complète des conducteurs avec distance approximative
SELECT 
  prenom, 
  nom, 
  telephone, 
  vehicle_type,
  vehicle_marque || ' ' || vehicle_modele || ' ' || vehicle_couleur as vehicule,
  vehicle_plaque,
  statut
FROM conducteurs 
ORDER BY vehicle_type, nom;