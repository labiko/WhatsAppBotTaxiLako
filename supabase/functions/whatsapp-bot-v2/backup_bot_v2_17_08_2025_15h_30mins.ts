...
const GREEN_API_TOKEN = Deno.env.get('GREEN_API_TOKEN') || '022e5da3d2e641ab99a3f70539270b187fbfa80635c44b71ad';
...
async function handleTextMessage(from: string, body: string, latitude?: string, longitude?: string): Promise<Response> {
  ...
  // Nouveau: Détecter si c'est un message de localisation (même vide)
  const isLocationMessage = false; // Sera défini lors du parsing
  const isTwilioLocation = false; // Sera défini lors du parsing
...
• "non" → Réservation pour quelqu'un d'autre`;
  ...
  } else if (isLocationMessage || isTwilioLocation) {
    if (hasLocation) {
      // ✅ GPS valide → traitement normal (passer au bloc suivant)
    } else {
      // ❌ GPS partagé mais coordonnées vides → message de repartage
      responseMessage = `❌ Position non reçue
...
serve(async (req) => {
  ...
  try {
    ...
    if (WHATSAPP_PROVIDER === 'greenapi' && contentType.includes('application/json') && requestData) {
      ...
      const isTwilioLocation = formData.has('Latitude') && formData.has('Longitude');
