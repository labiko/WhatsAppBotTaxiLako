...
Tapez le numéro ou le nom de votre destination:`;
      ...
      // SÉLECTION NUMÉRIQUE : Vérifier si l'utilisateur a tapé un chiffre (1-9)
      if (/^[1-9]$/.test(body.trim())) {
        const choiceIndex = parseInt(body.trim()) - 1;
        // Récupérer les mêmes suggestions que celles affichées précédemment
        const googleSuggestions = await getSuggestionsIntelligentes('', 10);
        const suggestions = await enrichSuggestionsWithPersonalAddresses(clientPhone, googleSuggestions, true);
        
        if (choiceIndex < suggestions.length) {
          adresse1 = suggestions[choiceIndex];
          console.log(`🎯 SÉLECTION NUMÉRIQUE: ${body} → ${adresse1.nom} (${adresse1.ville})`);
        } else {
          console.log(`❌ SÉLECTION NUMÉRIQUE: Index ${body} invalide (max: ${suggestions.length})`);
        }
      }
      
      // Si pas de sélection numérique ou invalide, faire recherche normale
      if (!adresse1) {
        console.log(`🔍 Recherche destination: "${body}"`);
        // ENRICHISSEMENT: Chercher d'abord dans les adresses personnelles, puis Google Places
        const googleResult = await searchAdresse(body);
        const googleResults = googleResult ? [googleResult] : [];
        const enrichedResults = await enrichSuggestionsWithPersonalAddresses(clientPhone, googleResults, true);
        adresse1 = enrichedResults.length > 0 ? enrichedResults[0] : null;
      }
      if (!adresse1) {
        responseMessage = `❓ Destination non trouvée: "${body}"
