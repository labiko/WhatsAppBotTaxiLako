...
async function handleTextMessage(from, body, latitude, longitude, payload = null) {
  ...
  // 🛡️ GARDE DE PROTECTION - Ignorer messages sans session active et sans commande taxi
  if (!session && !body.includes('taxi') && !body.includes('annuler')) {
    console.log(`🛡️ DEBUG - Aucune session active et message non-'taxi' → Message ignoré`);
