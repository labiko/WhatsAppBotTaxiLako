...
Ensuite, nous calculerons le prix pour ${session.destinationNom}.`;
...
**Tapez le nom de votre destination:**`;
          ...
          const adressesIA = await chercherAdresse(session.destinationNom);
          
          if (adressesIA.length > 0) {
            const adresseSelectionnee = adressesIA[0]; // Prendre la première correspondance
...
💰 **Prix estimé: ${prixInfo.prixTotal.toLocaleString()} GNF**

ℹ️ Tarif appliqué: ${prixInfo.tarifKm.toLocaleString()} GNF/km
