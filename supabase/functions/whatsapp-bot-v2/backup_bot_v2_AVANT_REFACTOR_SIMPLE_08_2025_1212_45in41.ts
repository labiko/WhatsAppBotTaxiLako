...
Tapez le numéro de votre choix ou essayez un autre nom`;
      ...
      // Choix multiples - utiliser la même logique que choix_depart_multiple
      await saveSession(clientPhone, {
        ...session,
        suggestionsDepart: JSON.stringify(lieuxDepart),
        etat: 'choix_depart_tierce_multiple'
      });
...
📝 Tapez le numéro de votre choix (1-${lieuxDepart.length})`;
      ...
      // Un seul lieu trouvé ou objet unique
...
Tapez le nom du lieu où vous voulez aller.`;
  ...
  // Handler pour destination après choix de départ multiple (réservation tierce)
  } else if (session.etat === 'tierce_attente_destination' && !hasLocation) {
...
Tapez le numéro ou essayez un autre nom`;
      ...
      await saveSession(clientPhone, {
        ...
        etat: 'choix_destination_tierce_multiple'
      });
...
📝 Tapez le numéro de votre choix (1-${destinationsResults.length})`;
      ...
      const departCoords = {
        latitude: parseFloat(session.departPosition!.match(/POINT\(([^)]+)\)/)?.[1]?.split(' ')[1] || '0'),
        longitude: parseFloat(session.departPosition!.match(/POINT\(([^)]+)\)/)?.[1]?.split(' ')[0] || '0')
      };
      
      const distanceKm = calculateDistance(departCoords.latitude, departCoords.longitude, destination.latitude, destination.longitude);
      const prixInfo = await calculerPrixCourse(session.vehicleType!, distanceKm);
...
Tapez le numéro ou essayez un autre nom`;
      ...
      // Choix multiples destinations - similaire aux autres handlers  
      await saveSession(clientPhone, {
        ...session,
        suggestionsDestination: JSON.stringify(destinationsResults),
        etat: 'choix_destination_lieu_depart_multiple'
      });
...
Pour recommencer: écrivez 'taxi'`;
      ...
      try {
        ...
        if (saveResponse.ok) {
          ...
          const tierceInfo = (session.etat === 'prix_calcule_tiers' || session.etat === 'prix_calcule_tierce')
            ? `👤 Pour: Une autre personne\n📍 Départ: ${session.departNom}\n`
            : '';
...
${suggestions.map((lieu, i) => `${i + 1}. **${lieu.nom}** (${lieu.ville})`).join('\n')}`;
  ...
  } else if (session.etat === 'choix_depart_tierce_multiple' && !hasLocation) {
    // Gestion choix multiple départs pour réservation tierce
    try {
      const suggestions = JSON.parse(session.suggestionsDepart || '[]');
      const choixNumero = parseInt(messageText);
      ...
      if (!isNaN(choixNumero) && choixNumero >= 1 && choixNumero <= suggestions.length) {
        ...
        // Vérifier conducteurs au lieu choisi
        const conducteursProches = await getAvailableDrivers(
          session.vehicleType!,
          {lat: departChoisi.latitude, lon: departChoisi.longitude},
          5000
        );
        
        if (conducteursProches.length === 0) {
          // Aucun conducteur au lieu choisi
          await saveSession(clientPhone, {
            ...session,
            departNom: departChoisi.nom,
            departId: departChoisi.id,
...
• Tapez "taxi" pour recommencer`;
          ...
          // Conducteurs trouvés - demander destination
          await saveSession(clientPhone, {
            ...session,
            departNom: departChoisi.nom,
            departId: departChoisi.id,
            departPosition: `POINT(${departChoisi.longitude} ${departChoisi.latitude})`,
            etat: 'tierce_attente_destination',
            suggestionsDepart: null
          });
...
${suggestions.map((lieu, i) => `${i + 1}. **${lieu.nom}** (${lieu.ville || 'Conakry'})`).join('\n')}`;
      ...
      console.error('❌ Erreur gestion choix multiple départs tierce:', error);
      responseMessage = `❌ Erreur technique. Retapez le nom du lieu de départ.`;
    }
...
${suggestions.map((lieu, i) => `${i + 1}. **${lieu.nom}** (${lieu.ville})`).join('\n')}`;
  ...
  } else if (session.etat === 'choix_destination_tierce_multiple' && !hasLocation) {
    // Gestion choix multiple destinations pour réservation tierce
    try {
      const suggestions = JSON.parse(session.suggestionsDestination || '[]');
      const choixNumero = parseInt(messageText);
      ...
      if (!isNaN(choixNumero) && choixNumero >= 1 && choixNumero <= suggestions.length) {
        ...
        const distanceKm = calculateDistance(departCoords.latitude, departCoords.longitude, destinationChoisie.latitude, destinationChoisie.longitude);
        const prixInfo = await calculerPrixCourse(session.vehicleType!, distanceKm);
...
${suggestions.map((lieu, i) => `${i + 1}. **${lieu.nom}** (${lieu.ville || 'Conakry'})`).join('\n')}`;
      ...
      console.error('❌ Erreur gestion choix multiple destinations tierce:', error);
      responseMessage = `❌ Erreur technique. Retapez le nom de la destination.`;
    }
