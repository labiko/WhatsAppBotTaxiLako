...
Tapez le nom de votre destination:`;
      ...
      } else if (session.etat === 'position_recue' && !session.destinationNom) {
        // 🔧 CORRECTION WORKFLOW POSITION TRANSFÉRÉE - Position reçue = destination d'ami
        console.log('🔧 CORRECTION WORKFLOW - Position transférée détectée comme DESTINATION');
        console.log(`📍 Coordonnées destination d'ami: ${lat}, ${lon}`);
        
        // ✅ TRAITEMENT POSITION TRANSFÉRÉE COMME DESTINATION
        if (session.positionClient) {
          console.log(`📍 Position client existante: ${session.positionClient}`);
          
          // 🧮 CALCULER DISTANCE entre position client et position destination (ami)
          const clientCoords = session.positionClient.split(',').map(coord => parseFloat(coord.trim()));
          const destLat = parseFloat(lat);
          const destLon = parseFloat(lon);
          const distanceKm = calculateDistance(clientCoords[0], clientCoords[1], destLat, destLon);
          
          console.log(`🧮 Distance calculée: ${distanceKm.toFixed(1)} km`);
          
          // 💰 CALCULER PRIX DE LA COURSE
          const prixInfo = await calculerPrixCourse(session.vehicleType, distanceKm);
          console.log(`💰 Prix calculé:`, prixInfo);
          
          // 🚗 TROUVER CONDUCTEUR LE PLUS PROCHE
          const conducteur = await findNearestDriver(session.vehicleType, clientCoords[0], clientCoords[1]);
          console.log(`🚗 Conducteur trouvé:`, conducteur);
          
          if (conducteur && prixInfo) {
            // 📱 FORMATER MESSAGE PRIX AVEC POSITION TRANSFÉRÉE
            const destPosition = `${destLat},${destLon}`;
            
            responseMessage = `💰 **PRIX CALCULÉ**

🚗 **${session.vehicleType.toUpperCase()}** - ${distanceKm.toFixed(1)} km
📍 **Destination**: Position d'ami partagée
💰 **Prix**: ${prixInfo.prix.toLocaleString()} ${prixInfo.devise}

👨‍🦰 **${conducteur.nom_complet}**
🏷️ ${conducteur.plaque_immatriculation}
⏱️ Arrivée dans ${conducteur.temps_arrivee_minutes} min
📞 ${conducteur.numero_telephone}

Confirmez-vous cette réservation ?
• "oui" = Réserver maintenant
• "non" = Annuler la demande`;

            // 💾 SAUVEGARDER SESSION AVEC DESTINATION POSITION TRANSFÉRÉE
            await saveSession(clientPhone, {
              ...session,
              destinationNom: 'Position d\'ami partagée',
              destinationPosition: destPosition,
              distanceKm: distanceKm,
              prixEstime: prixInfo.prix,
              etat: 'prix_calcule',
              conducteurAssigne: JSON.stringify(conducteur)
            });
            
            console.log(`✅ CORRECTION WORKFLOW - Position transférée traitée comme destination avec succès`);
          } else {
            responseMessage = `❌ Désolé, aucun conducteur ${session.vehicleType} disponible actuellement près de votre position.`;
            console.log(`❌ CORRECTION WORKFLOW - Pas de conducteur disponible`);
          }
        } else {
          console.log(`❌ CORRECTION WORKFLOW - Position client manquante`);
          responseMessage = `❌ Erreur : Position de départ manquante. Veuillez redémarrer avec "taxi".`;
        }
      } else {
  ...
  try {
    ...
    if (WHATSAPP_PROVIDER === 'greenapi' && contentType.includes('application/json') && requestData) {
      ...
      if (payload.typeWebhook === 'incomingMessageReceived') {
        ...
        // Gestion localisation Green API - Positions directes ET transférées
        if (messageData.locationMessageData) {
          const locationData = messageData.locationMessageData;
          
          // 📍 LOG DEBUG pour positions transférées
          if (locationData.isForwarded) {
            console.log('📍 CORRECTION - Position transférée détectée, traitement normal');
            console.log('📍 CORRECTION - Coordonnées transférées:', locationData.latitude, locationData.longitude);
          } else {
            console.log('📍 DEBUG TRANSFER - Position directe détectée');
          }
          
          console.log('📍 DEBUG TRANSFER - LocationData:', locationData);
          
          // ✅ EXTRACTION IDENTIQUE (directe OU transférée)
...
⚠️ Pas "Position en direct"`;
          ...
          // 📍 LOG SUCCÈS
          if (locationData.isForwarded) {
            console.log('✅ CORRECTION - Position transférée traitée avec succès');
          }
        }
        ...
        console.log(`  - Is forwarded: ${messageData?.extendedTextMessageData?.isForwarded}`);
