...
Tapez le numéro ou essayez un autre nom`;
      ...
      if (!session.lieuDepartPosition && !session.lieuDepartNom) {
      ...
      const addressData = session.lieuDepartPosition || session.lieuDepartNom;
...
• Répondez 'non' pour annuler`;
    ...
    const positionDepart = session.etat === 'prix_calcule_tiers' && (session.lieuDepartPosition || session.lieuDepartNom)
      ? await getCoordinatesFromAddress(session.lieuDepartPosition || session.lieuDepartNom!)
...
Pour recommencer: écrivez 'taxi'`;
      ...
      const departCoords = session.etat === 'prix_calcule_tiers' && (session.lieuDepartPosition || session.lieuDepartNom)
        ? await getCoordinatesFromAddress(session.lieuDepartPosition || session.lieuDepartNom!)
        : await getClientCoordinates(normalizePhone(from));
...
Veuillez réessayer plus tard.`;
  ...
  // Handler pour élargir le rayon de recherche
    ...
    const centerCoords = session.etat === 'aucun_conducteur_proximite' 
      ...
      : (session.lieuDepartPosition || session.lieuDepartNom)
        ? await getCoordinatesFromAddress(session.lieuDepartPosition || session.lieuDepartNom!)
        : await getClientCoordinates(clientPhone);
