...
/**
 * Analyse du message via GPT-4 (fonction stub pour l'instant)
 */
async function analyzeMessageWithAI(message: string): Promise<AIAnalysis | null> {
  console.log(`🧠 [AI_ANALYZE] Analyse GPT-4 du message: "${message}"`);
  
  // TODO: Implémenter l'appel réel à GPT-4
  // Pour l'instant, simulation basique pour tester la structure
  ...
  if (confidence < 0.7) {
    console.log(`🧠 [AI_ANALYZE] Confiance insuffisante: ${confidence}`);
    return null;
  }
...
async function handleSimpleReservation(analysis: AIAnalysis, clientPhone: string): Promise<IAResult> {
  ...
  // Créer message de réponse enrichi
...
/**
 * Fonction de détection de complexité des messages
 */
function shouldUseAIAnalysis(message: string): boolean {
  console.log(`🔍 [SHOULD_USE_IA] Analyse: "${message}"`);
  
  // Messages simples = pas d'IA
  if (message.length < 10) {
    console.log(`🔍 [SHOULD_USE_IA] Trop court (${message.length} < 10)`);
    return false;
  }
  
  const lowerMessage = message.toLowerCase();
  const simpleCommands = ['taxi', 'moto', 'voiture', 'oui', 'non', 'annuler', '1', '2', '3'];
  if (simpleCommands.includes(lowerMessage)) {
    console.log(`🔍 [SHOULD_USE_IA] Commande simple détectée: ${lowerMessage}`);
    return false;
  }
  ...
  if (wordCount >= 4) {
    console.log(`🔍 [SHOULD_USE_IA] Message complexe (${wordCount} mots >= 4)`);
    return true;
  }
  
  if (hasTemporalIndicators(message)) {
    console.log(`🔍 [SHOULD_USE_IA] Indicateurs temporels détectés`);
    return true;
  }
  
  if (hasDestinationPattern(message)) {
    console.log(`🔍 [SHOULD_USE_IA] Pattern destination détecté`);
    return true;
  }
  
  console.log(`🔍 [SHOULD_USE_IA] Message simple, pas d'IA nécessaire`);
  return false;
}

/**
 * Détection d'indicateurs temporels
 */
function hasTemporalIndicators(message: string): boolean {
  const temporalKeywords = [
    'demain', 'hier', 'aujourd\'hui', 'ce soir', 'ce matin', 'cet après-midi',
    'lundi', 'mardi', 'mercredi', 'jeudi', 'vendredi', 'samedi', 'dimanche',
    'à', 'vers', 'dans', 'heure', 'heures', 'min', 'minutes'
  ];
...
/**
 * Détection de patterns de destination
 */
function hasDestinationPattern(message: string): boolean {
  const destinationPatterns = [
    /\b(aller|vais|pour|vers)\s+[àau]?\s*\w+/i,
    /\b(hôpital|pharmacie|banque|restaurant|aéroport|université|marché)/i,
    /\b(madina|kipé|ratoma|kaloum|dixinn|matam)/i
  ];
  
  return destinationPatterns.some(pattern => pattern.test(message));
}
...
Distance: ${distance.toFixed(1)} km • ~${Math.ceil(distance * 3)} min

${temporalInfo}💰 **TARIF: ${prixInfo.prix_total.toLocaleString()} GNF**
