...
function isNonFrenchLanguage(message: string): boolean {
  const englishPatterns = /\b(I|want|to|the|airport|hello|hi|please|thank|you)\b/i;
...
async function handleSimpleReservation(analysis: AIAnalysis, clientPhone: string): Promise<IAResult> {
  ...
  // 🔍 CORRECTION V3→V2: Rechercher destination avec Google Places AVANT de sauvegarder
  console.log(`🔍 DEBUG - Recherche Google Places pour: "${analysis.destination}"`);
  ...
  if (!destinationResults || (Array.isArray(destinationResults) && destinationResults.length === 0)) {
    console.log(`❌ DEBUG - Aucun résultat pour: "${analysis.destination}"`);
    
    // 🔍 CORRECTION V3→V2: Ajouter suggestions intelligentes comme V2
    const suggestions = await getSuggestionsIntelligentes(analysis.destination || '', 5);
    const suggestionsText = suggestions.length > 0 
      ? suggestions.map((s, i) => `${i + 1}️⃣ ${s.nom} (${s.ville || 'Conakry'})`).join('\n')
      : `• CHU Donka (Conakry)
• Pharmacie Donka (Conakry)  
• Madina Centre (Conakry)
• Hôpital Ignace Deen (Conakry)
• Marché Niger (Conakry)`;
      
    return {
      handled: true,
      response: `❌ **DESTINATION INCONNUE**

🤖 J'ai analysé votre demande: "${analysis.destination}"

Cette destination n'existe pas ou n'est pas disponible.
...
Ou écrivez "taxi" pour le mode classique.`,
      analysis
    };
  }
  
  const destination = Array.isArray(destinationResults) ? destinationResults[0] : destinationResults;
  console.log(`✅ DEBUG - Destination trouvée: "${destination.nom}"`);
  
  // Sauvegarder session avec destination validée
  await saveSession(clientPhone, {
    vehicleType: analysis.vehicle_type === 'auto_detect' ? 'voiture' : analysis.vehicle_type,
    destinationNom: destination.nom, // ✅ Nom validé par Google Places
    destinationId: destination.id,
    destinationPosition: destination.latitude && destination.longitude ? 
      `POINT(${destination.longitude} ${destination.latitude})` : null,
...
async function processMessage(from: string, body: string, session: Session): Promise<Response> {
  ...
  if (shouldUseAIAnalysis(body)) {
    ...
    if (iaResult.handled) {
      ...
      // 🔄 UTILISER LES PROVIDERS WHATSAPP COMME DANS LE RESTE DU BOT
      // 🔄 CORRECTION CRITIQUE : Green API ne comprend pas Response(text/plain)
      if (WHATSAPP_PROVIDER === 'greenapi') {
        const messageSent = await sendGreenAPIMessage(from, iaResult.response);
        if (messageSent) {
          console.log(`✅ [IA] Message IA envoyé via Green API`);
          return new Response('OK', { status: 200, headers: corsHeaders });
        } else {
          console.log(`❌ [IA] Échec envoi Green API`);
 ...
 */
function formatDateForUser(dateString: string | null | undefined): string {
  // 🔧 CORRECTION : Gérer les valeurs null/undefined
  if (!dateString) {
    const today = new Date();
    return `Aujourd'hui ${today.getDate()}/${today.getMonth() + 1}`;
  }
...
async function createPrixCalculeMessage(session: Session, prixInfo: any, distance: number, destination: any): Promise<string> {
  ...
  // 🔧 CORRECTION DATE/HEURE : Gérer les valeurs null correctement
  let temporalInfo = '';
  if (session.temporalPlanning && session.plannedDate && session.plannedHour !== null) {
    temporalInfo = `📅 Date: ${formatDateForUser(session.plannedDate)} à ${session.plannedHour}:${(session.plannedMinute || 0).toString().padStart(2, '0')}\n`;
  } else if (session.temporalPlanning) {
    // Si planification mais données manquantes, afficher "Maintenant"
    const now = new Date();
    temporalInfo = `📅 Date: Maintenant (${now.getDate()}/${now.getMonth() + 1} à ${now.getHours()}:${now.getMinutes().toString().padStart(2, '0')})\n`;
  }
    ...
    if (await shouldUseAIAnalysis(messageText)) {
      ...
      if (aiResult.handled) {
        ...
        if (WHATSAPP_PROVIDER === 'greenapi') {
          ...
          if (messageSent) {
            console.log(`✅ [IA-INTEGRATION] Message IA envoyé via Green API`);
            return new Response('OK', { status: 200, headers: corsHeaders });
          } else {
            console.log(`❌ [IA-INTEGRATION] Échec envoi Green API`);
            return new Response('Error', { status: 500, headers: corsHeaders });
          }
        } else {
...
(Répondez par 'moto' ou 'voiture')`;
    ...
    // 🔍 DEBUG: Recherche destination avec logs détaillés
    console.log(`🔍 DEBUG IA_GPS - Recherche destination: "${session.destinationNom}"`);
    ...
    console.log(`🔍 DEBUG IA_GPS - Résultats searchLocation:`, JSON.stringify(destinationResults));
    
    // 🔧 CORRECTION #14: searchLocation() retourne un tableau - prendre le premier élément
    const destination = Array.isArray(destinationResults) ? destinationResults[0] : destinationResults;
    
    console.log(`🔍 DEBUG IA_GPS - Destination sélectionnée:`, JSON.stringify(destination));
    ...
    if (destination) {
      ...
      console.log(`🔍 DEBUG IA_GPS - AVANT sauvegarde:`);
      console.log(`  - session.destinationNom actuel: "${session.destinationNom}"`);
      console.log(`  - destination.nom enrichi: "${destination.nom}"`);
      
      // 🔧 CORRECTION: Mettre à jour la session avec le nom enrichi
      const updatedSession = {
        ...session,
        positionClient: `POINT(${longitude} ${latitude})`,
        destinationNom: destination.nom, // Utiliser le nom enrichi de Google Places
      ...
      console.log(`🔍 DEBUG IA_GPS - APRÈS construction updatedSession:`);
      console.log(`  - updatedSession.destinationNom: "${updatedSession.destinationNom}"`);
      
      await saveSession(clientPhone, updatedSession);
      
      console.log(`🔍 DEBUG IA_GPS - Session sauvegardée avec nom enrichi`);
      
      // Passer la session mise à jour avec le nom enrichi
      responseMessage = await createPrixCalculeMessage(updatedSession, prixInfo, distance, destination);
...
Pour recommencer: écrivez 'taxi'`;
      ...
      try {
        ...
        if (saveResponse.ok) {
          ...
          // 🔍 DEBUG: Vérifier pourquoi destinationNom n'est pas enrichi
          console.log(`🔍 DEBUG CONFIRMATION - session.destinationNom: "${session.destinationNom}"`);
          console.log(`🔍 DEBUG CONFIRMATION - session.destinationId: "${session.destinationId}"`);
          console.log(`🔍 DEBUG CONFIRMATION - session.destinationPosition: "${session.destinationPosition}"`);
